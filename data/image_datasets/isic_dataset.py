...
# Define ISIC2018 classes (Task 3 labels)
ISIC_CLASSES = ["MEL", "NV", "BCC", "AKIEC", "BKL", "DF", "VASC"]
CLASS_TO_IDX = {cls: idx for idx, cls in enumerate(ISIC_CLASSES)}
...
class ISIC2018_ImagesDataset(Dataset):
    def __init__(self, split: str, dataset_dir: str, label_dir: str, task_key: str, image_size=(224, 224), img_augmentation=False):
        ...
        self.split = split
        self.num_labels = len(ISIC_CLASSES)

        self.base_dir = os.path.join(dataset_dir, "ISIC2018")
        ...
        if "train" in split:
            ...
            label_csv_path = os.path.join(self.base_dir, "{0}_GroundTruth".format(split), label_dir, f"{label_task_key}.csv")
        elif "val" in split:
            label_csv_path = os.path.join(self.base_dir, "{0}_GroundTruth".format(split), "ISIC2018_Task3_Validation_GroundTruth.csv")
        elif "test" in split:
            label_csv_path = os.path.join(self.base_dir, "{0}_GroundTruth".format(split), "ISIC2018_Task3_Test_GroundTruth.csv")
        else:
            raise ValueError("ISIC2018 | Something wrong with the split!")
        ...
        self.image_dir = os.path.join(self.base_dir, "{0}_Image".format(split))
    ...
    def __getitem__(self, idx):
        ...
        label_vec = torch.tensor([label_data[cls] for cls in ISIC_CLASSES], dtype=torch.float32)
    ...
    def get_label_distribution(self):
        label_counts = np.zeros(len(ISIC_CLASSES), dtype=np.int32)
        for image_id in self.image_ids:
            label_data = self.label_dict[image_id]
            for idx, cls in enumerate(ISIC_CLASSES):
                if label_data[cls] == 1:
                    label_counts[idx] += 1
        return label_counts
...
def build_isic2018_dataloader(logger, args, split: str, label_dir: str, task_key: str, client_id=-1, **kwargs):
    logger.info(f"ISIC2018: Building dataloader for split='{split}', task='{task_key}'")

    shuffle = True if "train" in split else False
    image_size = getattr(args, "image_size", (224, 224))  # Default to (224, 224) for ISIC2018
    augmentation = getattr(args, "img_augmentation", False)

    dataset = ISIC2018_ImagesDataset(
        split=split,
