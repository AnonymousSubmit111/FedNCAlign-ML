...
class ResNetMultiHeadLearner(ContinualLearner):
    def __init__(self,
        ...
        """
        Multi-head ResNet-based learner for multiple clients/tasks.
        ...
        if self.class_wise_MLP:
            self.class_wise_MLP_layer = PerClassMLPs(num_classes=num_labels, input_dim=self.encoder_dim * num_images, hidden_dim=self.encoder_dim * 2, 
                                                     output_dim=self.encoder_dim * num_images, norm_type=norm)
    ...
    def add_task_layer(self, client_key: str, task_config: Dict):
        ...
        if (self.classifier_type == "FC_Classifier") or (self.classifier_type == "Dual_Classifier"):
            clf_layer = FCClassifierLayer(self.encoder_dim * num_images, num_images, num_labels, norm_type=self.norm)
            ...
            raise ValueError("resnet | Unsupported classifier type.")
