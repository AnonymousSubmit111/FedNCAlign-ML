...
class FeatureHook:
    def __init__(self, module):
        ...
        self.gbl  = torch.Tensor([])
        self.pca = PCA(n_components=16)
        
    def hook_fn(self, module, input, output):
 
        self.gbl  = self.gbl.to(output.device)
        output    = output.view(output.shape[0],-1)
        #print('OP_RESHAPE',output.shape)
        # tsne = TSNE(n_components=2)
        # tsne = tsne(output)
        output    = output.detach().cpu().numpy()
        #print('OP_SAMPLED',output.shape)
 
        #self.pca.fit(output[:,:])
        #output_pca = self.pca.transform(output)
        
        self.embedding = output #_pca
